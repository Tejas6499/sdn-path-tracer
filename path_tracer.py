...
class PathTracerController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.mac_to_loc = {}
        self.path_log = {}
        kwargs["wsgi"].register(RestController, {API_NAME: self})
    ...
    def pkt_in(self, ev):
        ...
        if key not in self.path_log:
            self.path_log[key] = {"path": path, "time": str(datetime.datetime.now())}
    ...
    def get_paths(self):
        return {str(k): v for k, v in self.path_log.items()}

    def get_macs(self):
        return {m: {"dpid": d, "port": p} for m, (d, p) in self.mac_to_loc.items()}

class RestController(ControllerBase):
    def __init__(self, req, link, data, **cfg):
        super().__init__(req, link, data, **cfg)
        self.app = data[API_NAME]

    @route("pt", "/pathtrace/paths", methods=["GET"])
    def paths(self, req, **kw):
        return Response(content_type="application/json",
                        body=json.dumps(self.app.get_paths(), indent=2))

    @route("pt", "/pathtrace/mactable", methods=["GET"])
    def macs(self, req, **kw):
        return Response(content_type="application/json",
                        body=json.dumps(self.app.get_macs(), indent=2))
